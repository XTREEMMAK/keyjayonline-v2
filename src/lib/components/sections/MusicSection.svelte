<script>
	...
	import { togglePlayer, showPlayer, loadPlaylist, loadRandomTrack, radioModalOpen, radioEnabled as radioEnabledStore } from '$lib/stores/musicPlayer.js';
			...
			<div class="container mx-auto px-4 text-center">
				...
				{#if musicNetworks && musicNetworks.length > 0}
					<div class="flex flex-wrap justify-center items-center gap-4 sm:gap-6 md:gap-8 max-w-4xl mx-auto">
						...
						{/each}
						{#if $radioEnabledStore}
							<button
								onclick={() => radioModalOpen.set(true)}
								class="group transition-all duration-300 hover:scale-110 opacity-70 hover:opacity-100"
								aria-label="Listen on Key Jay Radio"
							>
								<Icon icon="mdi:radio-tower" class="text-white text-6xl sm:text-7xl md:text-8xl" />
							</button>
						{/if}
					</div>
				{:else}
